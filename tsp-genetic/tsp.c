...
route_t solve_tsp_genetic(int population_size, int stopping_time,
                          thread_pool_t *pool, maxrandom_data_t *rnd,
                          int verbose);

void generate_route(void *arg, maxrandom_data_t *rnd);

void calculate_fitness(void *arg, maxrandom_data_t *rnd);

/* exchange 2 vertices (select 1-neighbour) */
void mutate(void *arg, maxrandom_data_t *rnd);

/* select random part of first parent and complete in order of second */
typedef struct {
  route_t *parent1, *parent2;
  route_t *child;
  int *buffer;
} crossover_args_t;
void crossover(void *arg, maxrandom_data_t *rnd);
...
int main(int argc, char **argv) {
  ...
  if (argc != 6 && (argc != 7 || !(verbose = !strcmp(argv[6], "--verbose")))) {
    fprintf(stderr, "usage: %s t N S { --generate n | --file FILENAME }"
                    " [--verbose]\n", argv[0]);
    return EXIT_FAILURE;
  }
  ...
  maxrandom_init(time(NULL) + 1, PACK_SIZE, thread_count + 2);
  maxrandom_data_init(&rnd);
  ...
  fprintf(stats, "%d %d %d %.2fs %d\n", thread_count, population_size,
          stopping_time, elapsed_time, solution.fitness);
...
route_t solve_tsp_genetic(int population_size, int stopping_time,
  ...
  route_t best = { malloc(graph->n * sizeof(int)), INT_MAX };
  ...
  while (improvement_time++ < stopping_time) {
    ...
    for (selected = 0; selected < selection_size; ++selected) {
      int u = maxrandom(rnd) % (population_size - selected);
      int v = maxrandom(rnd) % (population_size - selected);
    ...
    /* mutation stage */
    for (i = 0; i < population_size; ++i) {
      if ((double) maxrandom(rnd) / RAND_MAX < MUTATION_RATE) {
...
void generate_route(void *arg, maxrandom_data_t *rnd) {
  ...
  for (i = 0; i < graph->n; ++i) {
    int tmp = r->route[i];
    int j = i + maxrandom(rnd) % (graph->n - i);
    r->route[i] = r->route[j];
    r->route[j] = tmp;
  }
...
void mutate(void *arg, maxrandom_data_t *rnd) {
  route_t *r = (route_t *) arg;
  int i = maxrandom(rnd) % graph->n;
  int j = maxrandom(rnd) % graph->n;
...
void crossover(void *arg, maxrandom_data_t *rnd) {
  ...
  int start_1 = maxrandom(rnd) % graph->n;
  int len_1 = maxrandom(rnd) % graph->n;
  int i, j;
